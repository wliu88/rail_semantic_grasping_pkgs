...
class DataCollection:
    ...
    def __init__(self, collect_objects=True):
        ...
        if collect_objects:
            self.semantic_objects_with_grasps_topic = rospy.get_param("~semantic_objects_with_grasps_topic")
        ...
        if collect_objects:
            ...
            self.semantic_objects_with_grasps_sub = rospy.Subscriber(self.semantic_objects_with_grasps_topic,
                                                         SemanticObjectList, self.semantic_objects_with_grasps_callback)
            rospy.loginfo("Listen to semantic objects with grasps from {}".format(self.semantic_objects_with_grasps_topic))
        else:
            # Set up publishers
    ...
    def semantic_objects_with_grasps_callback(self, semantic_objects_with_grasps):
        """
        This method is used for listening to semantic objects with grasps and saves them to files.
        ...
        with open(object_file_path, "wb") as fh:
            pickle.dump(semantic_objects_with_grasps, fh)
